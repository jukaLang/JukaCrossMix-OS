...
while true; do
    ...
    1)
        ...
        if [ -f "/tmp/cmd_to_run.sh" ]; then
            ...
            echo -n "$CPU_FREQ_MAX" >/sys/devices/system/cpu/cpufreq/policy0/scaling_min_freq
            echo -n "$CPU_FREQ_MAX" >/sys/devices/system/cpu/cpufreq/policy0/scaling_max_freq
            normal_restored=false
        fi
        ;;
    0)
